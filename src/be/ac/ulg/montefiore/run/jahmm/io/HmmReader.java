...
{	
	...
	{
		CustomStreamTokenizer st = new CustomStreamTokenizer(reader);
		initSyntaxTable(st);
	...
	readState(CustomStreamTokenizer st, int nbStates,	int stateNb, double[] pi,
			double[][] a,
	...
	/**
	 * Reads some keywords out of a {@link CustomStreamTokenizer}. 
	 * 
	 * @param st A stream tokenizer.
	 * @param words The words to read, in the right order.
	 */
	static void readWords(CustomStreamTokenizer st, String... words)
	throws IOException, FileFormatException
	{
		for (String word : words) {
			st.nextToken();
			
			if (st.ttype == CustomStreamTokenizer.TT_WORD)
	...
	static double readNumber(CustomStreamTokenizer st)
	throws IOException, FileFormatException
	{
		st.nextToken();
		
		if (st.ttype != CustomStreamTokenizer.TT_NUMBER)
			throw new FileFormatException(st.lineno(),
	...
	static void initSyntaxTable(CustomStreamTokenizer st)
	{
		st.resetSyntax();
		st.parseNumbers();
		st.wordChars('a', 'z');
