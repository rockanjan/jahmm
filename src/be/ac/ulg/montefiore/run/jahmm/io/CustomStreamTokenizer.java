...
public class CustomStreamTokenizer {
	...
	private int read() throws IOException {
		if (reader != null) {
			...
			if (debug) {
				if (c == '\n') {
					System.out.println("Reading(" + lvlRead + ") [newLine]");
				} else {
					System.out.println("Reading(" + lvlRead + ") [" + (char) c + "]");
				}
			}
			return c;
		} else if (input != null) {
	...
	public int nextToken() throws IOException {
		...
		if ((ctype & CT_DIGIT) != 0) {
			...
			while (true) {
				if (c == '.' && seendot == 0) {
					...
					CustomStreamTokenizer token_int = null;
					if (reader != null) {
						token_int = new CustomStreamTokenizer(reader);
					} else {
						token_int = new CustomStreamTokenizer(input);
					}
					if (debug) {
						System.out.println("new Tokenizer (" + this.lvlRead + "), c=" + (char) c);
					}
					token_int.fixSyntax();
					token_int.lvlRead = this.lvlRead + 1;
					int type_int;
					type_int = token_int.nextToken();
					if (type_int != CustomStreamTokenizer.TT_NUMBER) {
						throw (new IOException("The Number looks like Sicentific but is not parsable"));
					}
					//now decode the number present
					peekc = token_int.peekc;

					decexp = decexp - (int) (token_int.nval);
		...
		if ((ctype & CT_ALPHA) != 0) {
			...
			do {
				...
				buf[i++] = (char) c;
				c = read();
				ctype = c < 0 ? CT_WHITESPACE : c < 256 ? ct[c]
						: CT_ALPHA;
